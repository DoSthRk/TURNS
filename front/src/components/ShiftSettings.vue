<template>
  <div class="shift-settings-page">
    ...
    <div class="settings-header">
      ...
      <add-advisor class="add-advisor-entry" @refresh="handleAdvisorRefresh"></add-advisor>
      </div>
    </div>
...
export default {
  ...
  methods: {
    async initPage() {
      await Promise.all([this.fetchConsultants(), this.loadTypeOptions()])
    ...
    async fetchConsultants() {
      try {
        ...
        this.consultant = source.map(item => ({ ...item, workStatus: item.status }))
      } catch (error) {
    ...
    updateConsultantData(data) {
      ...
      items.forEach(item => {
        ...
        if (index >= 0) {
          ...
          this.consultant.push({ ...item, workStatus: item.status })
        }
      })
      this.refreshSettingConsultants()
    },
    ...
    async restoreConsultantStatus(consultant) {
      try {
        ...
        const response = await request({
          url: '/restore',
          method: 'post',
          params: { consultantId: consultant.id, fromStatus: consultant.status }
        })
        if (response.data.code === 200) {
          consultant.status = 1
          ...
          const target = this.consultant.find(c => c.id === consultant.id)
          if (target) { target.status = 1; target.workStatus = 1; target.description = '' }
    ...
    async confirmPauseStatusChange() {
      ...
      try {
        const statusResponse = await request({
          url: '/status',
          method: 'post',
          params: { consultantId: consultant.id, status: 3 }
        })
        ...
        await request.post('/updateConsultants', consultant, { params: { operationType: 'updatePauseReason' } })
        const target = this.consultant.find(c => c.id === consultant.id)
        if (target) { target.status = 3; target.workStatus = 3; target.description = this.pauseReason }
        this.$message.success('状态更新成功')
      } catch (error) {
        consultant.workStatus = this.currentConsultantOldStatus
        console.error('状态更新失败:', error)
